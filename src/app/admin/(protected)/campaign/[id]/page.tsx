...
export default async function CampaignDetailsPage({
  ...
  const [campaignRes, metricsRes, usersRes, discountRes] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "user_name, phone, coupon_code, discount_value, status, redeemed_at",
      )
      .eq("campaign_id", campaignId)
  ...
  if (metricsRes.error)
    console.error("[CampaignDetail] metrics error:", metricsRes.error);
  if (usersRes.error)
    console.error("[CampaignDetail] users error:", usersRes.error);
  ...
  const totalDiscountIssued = allCoupons.reduce(
    (sum: number, c: Coupon) => sum + Number(c.discount_value || 0),
    0,
  );
  const claimedDiscountIssued = allCoupons
    .filter((c: Coupon) => c.status === "claimed")
    .reduce((sum: number, c: Coupon) => sum + Number(c.discount_value || 0), 0);
  const avgDiscount =
    allCoupons.length > 0
      ? Math.round(totalDiscountIssued / allCoupons.length)
      : 0;

  // Analytics metrics
  const impressions = allCoupons.length; // Total coupons generated (all statuses)
  const claimedCount = allCoupons.filter(
    (c: Coupon) => c.status === "claimed",
  ).length;
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            subtext="Verified (Mark as Claimed)"
          />
          <StatCard
            title="Unique Users"
            value={metrics.unique_users}
            subtext="From coupon creation"
          />
          <StatCard
            title="Conversion"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
          <StatCard
            title="Impressions"
            value={impressions}
            subtext="Total coupons generated"
          />
          <StatCard
      ...
      <section>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
          ...
          <span className="ml-2 text-sm font-normal text-gray-400">
            (all coupon holders — active &amp; claimed)
          </span>
        </h2>
              ...
              <tbody className="divide-y divide-gray-100">
                {users.length === 0 ? (
                  <tr>
                    ...
                    >
                      No users yet — they appear as soon as a coupon is
                      generated.
                    </td>
                  </tr>
                ) : (
                  users.map((u: CampaignUser, i: number) => (
                    <tr key={i} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-500 whitespace-nowrap">
                        {formatDate((u.redeemed_at || u.generated_at) ?? null)}
                      </td>
                    </tr>
                  ))
                )}
