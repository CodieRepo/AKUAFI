...
export async function POST(request: Request) {
  try {
    ...
    // 4. Update Database (Atomic - Update Bottle First)
    ...
    const { data: bottleUpdated, error: bottleUpdateError } = await supabaseAdmin
        ...
        .select()
        .single(); // Should return the row if updated, or error/null if not found/condition failed
    ...
    if (couponError) {
        ...
        // We could try to revert (optimistic) but it's risky.
        // For now, return error. Support would need to handle this edge case.
        return NextResponse.json({ error: 'Failed to generate coupon' }, { status: 500 });
    }

    // Removed campaign_leads logic as per instructions.
