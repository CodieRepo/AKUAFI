...
export default function CouponVerification({
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const query = supabase
        ...
        .select(
          "coupon_code, status, campaign_name, campaign_id, phone, user_name, redeemed_at, discount_value",
        )
      ...
      else if (data.status === "claimed" || data.status === "redeemed")
  ...
  const handleClaim = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to mark coupon ${couponData.coupon_code} as claimed?`,
      )
    )
      return;
      ...
      <div className="flex flex-col gap-3 mb-4">
        <div className="relative group">
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && code.length >= MIN_LENGTH && handleVerify()
            }
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                  {couponData?.notOwned ? (
                    ...
                    <>
                      The code{" "}
                      <span className="font-mono font-medium">{code}</span> does
        ...
        {status === "idle" && (
          <div className="h-full flex items-center justify-center p-4 text-center rounded-xl border border-dashed border-gray-200 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50">
            <p className="text-sm text-gray-400">
              Enter a coupon code above to verify its status.
            </p>
          </div>
        )}
      </div>
